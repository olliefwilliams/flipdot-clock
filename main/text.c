...
#include "include/fonts/font_3x5.h"
...
// almost identical copy of above, should probably merge the two at some point

void render_text_3x5(dotboard_t *dotboard, int x, int y, const char *text)
{
  // For each char in the requested text
  for (int i = 0; i < strlen(text); i++)
  {
    ...
    for (int c_row = 0; c_row < 5; c_row++)
    {
      for (int c_col = 0; c_col < 3; c_col++)
      {
        ...
        (*dotboard)[col][row] = (font_3x5[(int)text[i]][c_row] & (1 << (2 - c_col))) ? 1 : 0;
      }
    }
